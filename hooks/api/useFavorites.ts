...
export const useToggleFavoritePlace = () => {
    ...
    return useMutation({
        ...
        onMutate: async (id) => {
            await queryClient.cancelQueries({ queryKey: ["favorites", "isSaved", "place", id] });
            const previousIsSaved = queryClient.getQueryData<boolean>(["favorites", "isSaved", "place", id]);
            
            // Optimistically update to the new value
            queryClient.setQueryData(["favorites", "isSaved", "place", id], (old: boolean | undefined) => !old);
            
            return { previousIsSaved };
        },
        onError: (err, id, context) => {
            queryClient.setQueryData(["favorites", "isSaved", "place", id], context?.previousIsSaved);
            toast.error("Failed to update favorites");
        },
        onSuccess: (data, variables) => {
            queryClient.invalidateQueries({ queryKey: ["favorites", "places"] });
            queryClient.invalidateQueries({ queryKey: ["favorites", "isSaved", "place", variables] });
...
export const useToggleFavoriteEvent = () => {
    ...
    return useMutation({
        ...
        onMutate: async (id) => {
             await queryClient.cancelQueries({ queryKey: ["favorites", "isSaved", "event", id] });
             const previousIsSaved = queryClient.getQueryData<boolean>(["favorites", "isSaved", "event", id]);
             
             queryClient.setQueryData(["favorites", "isSaved", "event", id], (old: boolean | undefined) => !old);
             
             return { previousIsSaved };
        },
        onError: (err, id, context) => {
            queryClient.setQueryData(["favorites", "isSaved", "event", id], context?.previousIsSaved);
            toast.error("Failed to update favorites");
        },
        onSuccess: (data, variables) => {
