...
export const tripExtendedService = {
  ...
  getRecommendedPlaces: async (
    ...
    const response = await api.get<PaginatedResult<PlaceDto>>(
      `/trips/${tripId}/recommendations/places`,
      {
        params: { page, limit },
      },
    );
  ...
  getRecommendedEvents: async (
    ...
    const response = await api.get<PaginatedResult<EventDto>>(
      `/trips/${tripId}/recommendations/events`,
      {
        params: { page, limit },
      },
    );
  ...
  getPlacesInTripProvinces: async (
    ...
    const response = await api.post<PaginatedResult<PlaceDto>>(
      `/trips/${tripId}/places`,
      {
        page: filters.page || 1,
  ...
  addItemToTripDay: async (
    tripId: number,
    dayNumber: number,
    item: CreateTripDayItemRequest,
  ) => {
    const response = await api.post(
      `/trips/${tripId}/days/${dayNumber}/items`,
      item,
    );
    return response.data;
  },
  ...
  updateTripDayItem: async (
    ...
    const response = await api.put(
      `/trips/${tripId}/days/${dayNumber}/items/${itemId}`,
      updates,
    );
    return response.data;
  },

  removeTripDayItem: async (
    tripId: number,
    dayNumber: number,
    itemId: number,
  ) => {
  ...
  getEventsInTripProvinces: async (
    ...
    const response = await api.post<PaginatedResult<EventDto>>(
      `/trips/${tripId}/events`,
      {
        page: filters.page || 1,
        limit: filters.limit || 8,
