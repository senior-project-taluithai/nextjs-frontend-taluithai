...
const SEASON_COLOR: Record<string, string> = {
    winter: "bg-sky-500/90 text-white", summer: "bg-amber-500/90 text-white", rainy: "bg-teal-500/90 text-white", all_year: "bg-emerald-500/90 text-white"
};
const SEASON_LABEL: Record<string, string> = { winter: "Winter", summer: "Summer", rainy: "Rainy", all_year: "All Year" };

const CAT_ACCENT: Record<string, { bg: string; text: string; border: string; dot: string; glow: string }> = {
    Temple: { bg: "bg-amber-50 dark:bg-amber-950/30", text: "text-amber-700 dark:text-amber-400", border: "border-amber-200 dark:border-amber-800", dot: "#f59e0b", glow: "rgba(245,158,11,0.2)" },
    Beach: { bg: "bg-sky-50 dark:bg-sky-950/30", text: "text-sky-700 dark:text-sky-400", border: "border-sky-200 dark:border-sky-800", dot: "#0ea5e9", glow: "rgba(14,165,233,0.2)" },
    Nature: { bg: "bg-emerald-50 dark:bg-emerald-950/30", text: "text-emerald-700 dark:text-emerald-400", border: "border-emerald-200 dark:border-emerald-800", dot: "#10b981", glow: "rgba(16,185,129,0.2)" },
    Heritage: { bg: "bg-rose-50 dark:bg-rose-950/30", text: "text-rose-700 dark:text-rose-400", border: "border-rose-200 dark:border-rose-800", dot: "#f43f5e", glow: "rgba(244,63,94,0.2)" },
    Island: { bg: "bg-teal-50 dark:bg-teal-950/30", text: "text-teal-700 dark:text-teal-400", border: "border-teal-200 dark:border-teal-800", dot: "#14b8a6", glow: "rgba(20,184,166,0.2)" },
    Festival: { bg: "bg-violet-50 dark:bg-violet-950/30", text: "text-violet-700 dark:text-violet-400", border: "border-violet-200 dark:border-violet-800", dot: "#8b5cf6", glow: "rgba(139,92,246,0.2)" },
    Shopping: { bg: "bg-pink-50 dark:bg-pink-950/30", text: "text-pink-700 dark:text-pink-400", border: "border-pink-200 dark:border-pink-800", dot: "#ec4899", glow: "rgba(236,72,153,0.2)" },
    Market: { bg: "bg-orange-50 dark:bg-orange-950/30", text: "text-orange-700 dark:text-orange-400", border: "border-orange-200 dark:border-orange-800", dot: "#f97316", glow: "rgba(249,115,22,0.2)" },
};
...
function Tilt3DCard({ children, className = "", intensity = 6 }: { children: React.ReactNode; className?: string; intensity?: number }) {
    ...
    const rotateX = useSpring(useTransform(ry, [-0.5, 0.5], [intensity, -intensity]), { stiffness: 300, damping: 28 });
    const rotateY = useSpring(useTransform(rx, [-0.5, 0.5], [-intensity, intensity]), { stiffness: 300, damping: 28 });
    ...
    return (
        <motion.div ref={ref} onMouseMove={onMove} onMouseEnter={() => setHovered(true)}
            ...
            <AnimatePresence>
                {hovered && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        style={{ background: `radial-gradient(ellipse at ${glarePos.x}% ${glarePos.y}%, rgba(255,255,255,0.18) 0%, rgba(16,185,129,0.06) 30%, transparent 60%)`, mixBlendMode: "overlay" }} />
                )}
            </AnimatePresence>
        </motion.div>
    );
...
export function ExploreCard({ item, type, index = 0 }: ExploreCardProps) {
    ...
    const category = (item.categories && item.categories.length > 0) ? item.categories[0] : (type === 'place' ? 'Place' : 'Event');
            ...
            <Tilt3DCard intensity={7} className="h-full group">
                ...
                >
                    ...
                    <div className="relative h-52 overflow-hidden shrink-0">
                        <motion.img src={imageUrl} alt={title}
                        ...
                        {bestSeason && SEASON_COLOR[bestSeason] && (
                            <motion.span initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.2 + index * 0.03 }}
                                className={`absolute top-3 left-3 text-xs px-2.5 py-1 rounded-full backdrop-blur-xl ${SEASON_COLOR[bestSeason]} z-10`}
                                style={{ fontWeight: 600, boxShadow: "0 2px 12px rgba(0,0,0,0.2)" }}>
                                {SEASON_OPTIONS.find(s => s.id === bestSeason)?.icon} {SEASON_LABEL[bestSeason]}
                            </motion.span>
                        )}

                        {type === 'event' && (item as Event).start_date && (
                            <motion.div
                                initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.2 + index * 0.03 }}
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-4 z-10 pointer-events-none">
                            {/* Category pill */}
                            <span className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-white/15 backdrop-blur-md text-white border border-white/20 mb-2 pointer-events-auto shadow-sm"
                            ...
                            {/* Name + Subtitle (Thai) */}
                            <h3 className="text-white mb-0.5 truncate drop-shadow-md" style={{ fontWeight: 800, fontSize: "1.05rem" }}>{title}</h3>
                            <p className="text-white/80 text-xs truncate drop-shadow-sm">{subtitle}</p>
                        </div>
                    ...
                    <div className="p-4 relative flex-1 flex flex-col z-10">
                        ...
                        <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 text-xs">
                                ...
                                <span className="line-clamp-1">{province?.name_en || 'Unknown Location'}</span>
                            </div>
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-4 flex-1">
                            {tags.slice(0, 3).map(tag => (
                                <span key={tag} className="text-xs bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 px-2.5 py-1 rounded-lg border border-slate-100 dark:border-slate-800" style={{ fontWeight: 500 }}>
                                    {tag}
                                </span>
                            ))}
                            {tags.length > 3 && (
                                <span className="text-xs text-slate-400 px-1 py-1">+{tags.length - 3}</span>
                            )}
                        </div>

                        {/* CTA */}
                        <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
                            onClick={e => { e.stopPropagation(); router.push(`/${type}/${item.id}`); }}
                            ...
                            style={{ fontWeight: 700, background: "linear-gradient(135deg, #059669, #0d9488)", color: "white", boxShadow: "0 4px 14px rgba(5,150,105,0.3)" }}>
