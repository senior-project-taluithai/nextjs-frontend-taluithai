...
const SEASON_INFO: Record<
  string,
  { icon: React.ReactNode; label: string; labelTh: string; color: string }
> = {
  cool: {
...
export default function PlaceDetailPage({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "reviews" | "photos" | "tiktok"
  >("overview");
  ...
  useEffect(() => {
    if (place)
      interactionService.track({ place_id: placeId, interaction_type: "view" });
  }, [place, placeId]);
  ...
  if (isLoadingPlace)
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f4f6f8] text-emerald-600">
  ...
  const allImages = (place.image_urls || []).filter(
    (u) => u && u.trim() !== "",
  );
  if (
    allImages.length === 0 &&
    place.thumbnail_url &&
    place.thumbnail_url.trim() !== ""
  ) {
    allImages.push(place.thumbnail_url);
  }
  const coverImage =
    place.thumbnail_url && place.thumbnail_url.trim() !== ""
      ? place.thumbnail_url
  ...
  const googleRating = Math.round((place?.rating || 0) * 10) / 10;
  const userRatingDisplay = Math.round((place?.user_rating || 0) * 10) / 10;
  const userReviewCount = place?.user_rating_count || 0;
  const rank = Math.floor(Math.random() * 5) + 1;

  // Calculate rating distribution from actual user reviews
  const ratingDistribution = (() => {
    const reviews = place?.place_reviews || [];
    if (reviews.length === 0) {
    ...
    >
      ...
      <div className="relative h-[480px] overflow-hidden">
        ...
        <div
          ...
          style={{
            background:
              "repeating-linear-gradient(90deg, #10b981 0px, #10b981 8px, #0d9488 8px, #0d9488 16px, #059669 16px, #059669 24px)",
          }}
        />
              ...
              <div className="flex items-center gap-1.5">
                <div className="flex">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <Star
                      key={s}
                      className={`w-3.5 h-3.5 ${s <= Math.floor(googleRating) ? "text-amber-400 fill-amber-400" : "text-white/20 fill-white/20"}`}
                    />
                  ))}
                </div>
                <span
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-gray-500">
                    {googleRating} Google ·{" "}
                    {userReviewCount > 0
                      ? `${userRatingDisplay} (${userReviewCount} reviews)`
                      : ""}{" "}
                    · {province?.name_en}
                  </span>
                </div>
          ...
          <div>
                ...
                {[
                  {
                    ...
                    label: "Ratings",
                    value: `${googleRating} / ${userReviewCount > 0 ? userRatingDisplay : "-"}`,
                    sub: `Google / ${userReviewCount > 0 ? userReviewCount : 0} reviews`,
                  },
                  {
                  ...
                  <Tilt3DCard key={i} className="cursor-default" intensity={4}>
                    <div className="rounded-xl p-3 hover:bg-gray-50/80 transition-colors text-center group h-full flex flex-col items-center justify-center">
                      ...
                      <p
                        className="text-gray-900 group-hover:text-emerald-700 transition-colors"
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                <div className="flex items-center gap-2 bg-white rounded-xl px-4 py-2.5 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                  <div
                    ...
                    style={{
                      backgroundColor: seasonInfo.color + "18",
                ...
                <button className="flex items-center gap-2 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-xl px-4 py-2.5 text-white shadow-md shadow-emerald-200/50 hover:shadow-lg hover:shadow-emerald-200/50 transition-all group opacity-50 cursor-not-allowed">
            ...
            <StaggerItem index={2}>
              <div className="flex gap-1 bg-white rounded-2xl p-1.5 mb-6 shadow-sm border border-gray-100 overflow-x-auto scrollbar-hide">
                {(["overview", "reviews", "photos", "tiktok"] as const).map(
                  (tab) => (
                    <button
                      key={tab}
                    ...
                    >
                      ...
                      {tab === "reviews" && (
                        ...
                        >
                          {userReviewCount.toLocaleString()}
                        </span>
                      )}
                    </button>
                  ),
                    ...
                    <div className="flex items-center gap-8">
                      <div className="text-center">
                        ...
                        <div className="flex justify-center mb-1">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <Star
                              key={s}
                              className={`w-4 h-4 ${s <= Math.floor(googleRating) ? "text-amber-400 fill-amber-400" : "text-gray-200 fill-gray-200"}`}
                            />
                          ))}
                        </div>
                        {userReviewCount > 0 && (
                          <div className="text-center mt-4 pt-4 border-t border-gray-100">
                            ...
                            <div className="flex justify-center mb-1">
                              {[1, 2, 3, 4, 5].map((s) => (
                                <Star
                                  key={s}
                                  className={`w-3 h-3 ${s <= Math.floor(userRatingDisplay) ? "text-emerald-400 fill-emerald-400" : "text-gray-200 fill-gray-200"}`}
                                />
                              ))}
                            </div>
                            <p className="text-xs text-gray-500">
                              Website ({userReviewCount} reviews)
                            </p>
                          </div>
                        )}
                      </div>
                      <div className="flex-1 space-y-2">
                        {[5, 4, 3, 2, 1].map((s) => {
                          ...
                          const pct =
                            userReviewCount > 0
                              ? Math.round((count / userReviewCount) * 100)
                              : 0;
                          return (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      onClick={() =>
                        window.open(
                          `https://www.tiktok.com/search?q=${encodeURIComponent(place.name_en)}`,
                          "_blank",
                        )
                      }
                  ...
                  {isLoadingTiktok ? (
                    ...
                    <div className="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm text-center">
                      ...
                      <p className="text-gray-400 text-sm mt-1">
                        There aren't any videos tagged with this place yet.
                      </p>
                    </div>
                  )}
                </motion.div>
              ...
              <Tilt3DCard intensity={2}>
                <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg transition-shadow">
                  <div className="relative h-[250px] overflow-hidden z-0">
                    <Map
                      pos={[place.latitude, place.longitude]}
                      className="h-full w-full"
                      popupContent={place.name_en}
                    />
