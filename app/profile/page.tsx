...
import {
    User, Mail, Shield, Key, Eye, EyeOff, Check, X, Camera,
    MapPin, Mountain, Waves, TreePine, Landmark, Utensils,
    ChevronRight, Sparkles, Globe, Calendar, Heart, Star,
    Settings, Bell, LogOut, Award, TrendingUp, Compass,
    EyeOff as EyeOffIcon,
} from 'lucide-react';

const heroImage = "https://images.unsplash.com/photo-1760638096645-be1efc495937?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080";

const TRAVEL_PREFERENCES = [
    { id: "1", name: "วัดและโบราณสถาน", nameEn: "Temples & Heritage", icon: Landmark, color: "#f59e0b" },
    { id: "2", name: "ภูเขาและป่าไม้", nameEn: "Mountains & Forest", icon: Mountain, color: "#22c55e" },
    { id: "3", name: "ทะเลและชายหาด", nameEn: "Beach & Sea", icon: Waves, color: "#3b82f6" },
    { id: "4", name: "อาหารท้องถิ่น", nameEn: "Local Food", icon: Utensils, color: "#ef4444" },
    { id: "5", name: "ธรรมชาติ", nameEn: "Nature", icon: TreePine, color: "#10b981" },
    { id: "6", name: "ผจญภัย", nameEn: "Adventure", icon: Compass, color: "#8b5cf6" },
    { id: "7", name: "วัฒนธรรม", nameEn: "Culture", icon: Globe, color: "#ec4899" },
    { id: "8", name: "ถ่ายรูป", nameEn: "Photography", icon: Camera, color: "#06b6d4" },
    { id: "9", name: "เทศกาล", nameEn: "Festivals", icon: Calendar, color: "#f97316" },
    { id: "10", name: "Hidden Gem", nameEn: "Hidden Gems", icon: Sparkles, color: "#eab308" },
];
...
const NOTIFICATION_SETTINGS = [
    { id: "new_places", label: "สถานที่ใหม่", desc: "แจ้งเตือนเมื่อมีสถานที่ใหม่ในภูมิภาคที่สนใจ" },
            ...
            ([entry]) => {
                if (entry.isIntersecting && !started.current) {
                    ...
                    const animate = (now: number) => {
                        ...
                        if (elapsed >= 1) { setCount(value); return; }
...
function ChangePasswordDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    const validate = () => {
        ...
        else if (!/[A-Z]/.test(newPassword)) e.new = "ต้องมีตัวพิมพ์ใหญ่อย่างน้อย 1 ตัว";
        else if (!/[a-z]/.test(newPassword)) e.new = "ต้องมีตัวพิมพ์เล็กอย่างน้อย 1 ตัว";
        else if (!/[0-9]/.test(newPassword)) e.new = "ต้องมีตัวเลขอย่างน้อย 1 ตัว";
        else if (!/[^A-Za-z0-9]/.test(newPassword)) e.new = "ต้องมีสัญลักษณ์อย่างน้อย 1 ตัว";
    ...
    const strengthLabel = ["", "อ่อนมาก", "อ่อน", "ปานกลาง", "แข็งแรง", "แข็งแรงมาก"][passwordStrength];
                ...
                >
                    ...
                    >
                        <div className="relative bg-gradient-to-r from-[#0f1923] to-[#1a2f3f] p-6 pb-5">
                            ...
                            <div className="relative flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="text-white" style={{ fontSize: 18 }}>เปลี่ยนรหัสผ่าน</h3>
                                        <p className="text-white/50" style={{ fontSize: 13 }}>Change Password</p>
                                    </div>
                                </div>
                        ...
                        <div className="p-6 space-y-5">
                            <div>
                                <label className="text-gray-700 mb-1.5 block" style={{ fontSize: 13 }}>รหัสผ่านปัจจุบัน</label>
                                ...
                                {errors.current && <p className="text-red-500 mt-1" style={{ fontSize: 12 }}>{errors.current}</p>}
                            </div>

                            <div>
                                <label className="text-gray-700 mb-1.5 block" style={{ fontSize: 13 }}>รหัสผ่านใหม่</label>
                                <div className="relative">
                                    <input
                                ...
                                {errors.new && <p className="text-red-500 mt-1" style={{ fontSize: 12 }}>{errors.new}</p>}
                                ...
                                <div className="mt-2 space-y-1">
                                    {[
                                        ...
                                        { ok: /[A-Z]/.test(newPassword), text: "ตัวพิมพ์ใหญ่ (A-Z)" },
                                        { ok: /[a-z]/.test(newPassword), text: "ตัวพิมพ์เล็ก (a-z)" },
                                        { ok: /[0-9]/.test(newPassword), text: "ตัวเลข (0-9)" },
                                        { ok: /[^A-Za-z0-9]/.test(newPassword), text: "สัญลักษณ์ (!@#$...)" },
                                    ].map((r, i) => (
                                        <div key={i} className="flex items-center gap-1.5" style={{ fontSize: 11 }}>
                                            ...
                                            <span className={r.ok ? "text-emerald-600" : "text-gray-400"}>{r.text}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div>
                                <label className="text-gray-700 mb-1.5 block" style={{ fontSize: 13 }}>ยืนยันรหัสผ่านใหม่</label>
                                <div className="relative">
                                    <input
                                    ...
                                    <button
                                        ...
                                        className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
                                    >
                                        {showConfirm ? <EyeOffIcon className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                                    </button>
                                </div>
                                {errors.confirm && <p className="text-red-500 mt-1" style={{ fontSize: 12 }}>{errors.confirm}</p>}
                                {confirmPassword && newPassword === confirmPassword && (
                                    <p className="text-emerald-500 mt-1 flex items-center gap-1" style={{ fontSize: 12 }}>
...
function StatCard({ icon: Icon, label, value, color, delay }: {
    ...
    return (
        ...
        >
            ...
            <p className="text-gray-500" style={{ fontSize: 13 }}>{label}</p>
        </motion.div>
    );
}

function PreferenceChip({ pref, selected, onClick, delay }: {
    pref: typeof TRAVEL_PREFERENCES[0]; selected: boolean; onClick: () => void; delay: number;
}) {
    ...
    return (
        ...
        >
            ...
            >
                <Icon className="w-4 h-4" style={{ color: selected ? pref.color : "#9ca3af" }} />
            </div>
            <div className="text-left">
                <p style={{ fontSize: 13, color: selected ? "#0f1923" : "#6b7280", fontWeight: 500 }}>{pref.name}</p>
                <p style={{ fontSize: 11, color: selected ? "#6b7280" : "#9ca3af" }}>{pref.nameEn}</p>
            </div>
            {selected && (
                <motion.div
                    initial={{ scale: 0 }}
...
function RegionToggle({ region, selected, onClick }: {
    region: typeof REGIONS[0]; selected: boolean; onClick: () => void;
}) {
    return (
        <motion.button
        ...
        >
            ...
            <p style={{ fontSize: 11, color: selected ? "#059669" : "#9ca3af" }}>{region.labelEn}</p>
        </motion.button>
    );
}
...
export default function ProfilePage() {
    ...
    const savePreferences = async () => {
        ...
        try {
            ...
            toast.success("บันทึกความชอบในการเดินทางเรียบร้อย!");
        } catch (error: any) {
    ...
    const tabs = [
        ...
        { id: "preferences" as const, label: "ความชอบ", labelEn: "Preferences", icon: Heart },
        { id: "notifications" as const, label: "แจ้งเตือน", labelEn: "Notifications", icon: Bell },
    ];
            ...
            <div className="relative h-[320px] overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 right-0 h-8">
                    <svg viewBox="0 0 1200 32" className="w-full h-full" preserveAspectRatio="none">
                        <path d="M0,32 L0,24 Q30,0 60,24 Q90,32 120,24 Q150,0 180,24 Q210,32 240,24 Q270,0 300,24 Q330,32 360,24 Q390,0 420,24 Q450,32 480,24 Q510,0 540,24 Q570,32 600,24 Q630,0 660,24 Q690,32 720,24 Q750,0 780,24 Q810,32 840,24 Q870,0 900,24 Q930,32 960,24 Q990,0 1020,24 Q1050,32 1080,24 Q1110,0 1140,24 Q1170,32 1200,24 L1200,32Z" fill="#f8f9fa" />
                    </svg>
                </div>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    >
                        ...
                        >
                            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-emerald-400 to-teal-600 flex items-center justify-center text-white shadow-xl shadow-emerald-500/30 ring-4 ring-white/20 overflow-hidden">
                                {user.avatarUrl ? (
                                    ...
                                    <span style={{ fontSize: 36, fontWeight: 700 }}>{user.firstName?.charAt(0)}</span>
                                )}
                            </div>
                            <motion.div
                        ...
                        <h1 className="text-white mb-1" style={{ fontSize: 24, fontWeight: 700 }}>
                            {user.firstName} {user.lastName}
                        </h1>
                        <p className="text-white/50 flex items-center gap-1.5" style={{ fontSize: 14 }}>
                        ...
                        <div className="flex items-center gap-6 mt-5">
                            {STATS.map((s, i) => (
                                ...
                                >
                                    <div className="text-white" style={{ fontSize: 22, fontWeight: 700 }}>
                                        <AnimatedCounter value={s.value} />
                                    </div>
                                    <p className="text-white/40" style={{ fontSize: 12 }}>{s.label}</p>
                                </motion.div>
                            ))}
                        </div>
                    </motion.div>
                        ...
                        >
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                                <div className="px-6 pt-6 pb-4 border-b border-gray-50">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h2 style={{ fontSize: 18, fontWeight: 600, color: "#0f1923" }}>ข้อมูลบัญชี</h2>
                                            <p className="text-gray-400" style={{ fontSize: 13 }}>Account Information</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="p-6 space-y-5">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                        <div>
                                            <label className="text-gray-500 mb-1.5 block" style={{ fontSize: 12, fontWeight: 500 }}>ชื่อ (First Name)</label>
                                            <div className="h-11 rounded-xl border border-gray-100 bg-gray-50 px-4 flex items-center text-gray-700" style={{ fontSize: 14 }}>
                                                {user.firstName}
                                            </div>
                                        </div>
                                        <div>
                                            <label className="text-gray-500 mb-1.5 block" style={{ fontSize: 12, fontWeight: 500 }}>นามสกุล (Last Name)</label>
                                            <div className="h-11 rounded-xl border border-gray-100 bg-gray-50 px-4 flex items-center text-gray-700" style={{ fontSize: 14 }}>
                                                {user.lastName}
                                            </div>
                                        </div>
                                    ...
                                    <div>
                                        <label className="text-gray-500 mb-1.5 block" style={{ fontSize: 12, fontWeight: 500 }}>อีเมล (Email)</label>
                                        <div className="h-11 rounded-xl border border-gray-100 bg-gray-50 px-4 flex items-center text-gray-700 gap-2" style={{ fontSize: 14 }}>
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                                <div className="px-6 pt-6 pb-4 border-b border-gray-50">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h2 style={{ fontSize: 18, fontWeight: 600, color: "#0f1923" }}>ความปลอดภัย</h2>
                                            <p className="text-gray-400" style={{ fontSize: 13 }}>Security Settings</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="p-6">
                                    <div className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-gray-50 to-gray-50/50 border border-gray-100 group hover:border-emerald-200 hover:from-emerald-50/50 transition-all">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p style={{ fontSize: 14, fontWeight: 500, color: "#0f1923" }}>รหัสผ่าน</p>
                                                <p className="text-gray-400" style={{ fontSize: 12 }}>เปลี่ยนรหัสผ่านเพื่อความปลอดภัย</p>
                                            </div>
                                        </div>
                                        <motion.button
                            ...
                            <div>
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    <h2 style={{ fontSize: 18, fontWeight: 600, color: "#0f1923" }}>สถิติการท่องเที่ยว</h2>
                                    <span className="text-gray-400" style={{ fontSize: 13 }}>Travel Stats</span>
                                </div>
                                ...
                                <div className="px-6 pt-6 pb-4 border-b border-gray-50">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <h2 style={{ fontSize: 18, fontWeight: 600, color: "#0f1923" }}>ความชอบในการเดินทาง</h2>
                                                <p className="text-gray-400" style={{ fontSize: 13 }}>Travel Preferences</p>
                                            </div>
                                        </div>
                                            ...
                                            <div className="flex">
                                                {[...Array(MAX_SELECTION)].map((_, i) => (
                                                    <div
                                                        ...
                                                        style={{
                                                            background: i < selectedPreferenceIds.length ? "#10b981" : "#e5e7eb",
                                                        }}
                                                    />
                                                ))}
                                            </div>
                                    ...
                                    <p className="text-gray-400 mt-2 ml-[52px]" style={{ fontSize: 13 }}>
                                        เลือกสูงสุด {MAX_SELECTION} หมวดหมู่เพื่อปรับแต่งคำแนะนำของคุณ
                                    </p>
                                </div>

                                <div className="p-6">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        {TRAVEL_PREFERENCES.map((pref, i) => (
                                            <PreferenceChip
                                                key={pref.id}
                                                pref={pref}
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                                <div className="px-6 pt-6 pb-4 border-b border-gray-50">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h2 style={{ fontSize: 18, fontWeight: 600, color: "#0f1923" }}>ภูมิภาคที่สนใจ</h2>
                                            <p className="text-gray-400" style={{ fontSize: 13 }}>Preferred Regions</p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                                <div className="px-6 pt-6 pb-4 border-b border-gray-50">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h2 style={{ fontSize: 18, fontWeight: 600, color: "#0f1923" }}>การแจ้งเตือน</h2>
                                            <p className="text-gray-400" style={{ fontSize: 13 }}>Notification Preferences</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="p-6 space-y-3">
                                    {NOTIFICATION_SETTINGS.map((setting, i) => (
                                        ...
                                        >
                                            <div>
                                                <p style={{ fontSize: 14, fontWeight: 500, color: "#0f1923" }}>{setting.label}</p>
                                                <p className="text-gray-400" style={{ fontSize: 12 }}>{setting.desc}</p>
                                            </div>
                                            <button
                                                ...
                                                className={`relative w-12 h-7 rounded-full transition-all ${
                                                    notifications[setting.id] ? "bg-emerald-500" : "bg-gray-200"
                                                }`}
                                            >
                                                <motion.div
