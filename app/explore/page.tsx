...
import { ExploreCalendar } from "@/components/explore/explore-calendar";
...
const HERO_PARTICLES = [
  ...
  { delay: 2, size: 4, x: "25%", dur: 10 },
  { delay: 4, size: 8, x: "45%", dur: 7 },
  { delay: 1, size: 5, x: "65%", dur: 9 },
  { delay: 3, size: 7, x: "80%", dur: 11 },
  { delay: 5, size: 3, x: "92%", dur: 8 },
];
...
export default function ExplorePage() {
  ...
  const placeQuery: ExplorePlacesQuery = useMemo(() => {
    ...
    return {
      ...
      provinces:
        (filters.provinceIds?.length ?? 0) > 0
          ? filters.provinceIds
          : undefined,
      categoryId: filters.categoryId,
      bestSeason:
        (filters.bestSeason?.length ?? 0) > 0 ? filters.bestSeason : undefined,
  ...
  const sortLabel =
    SORT_OPTIONS.find((s) => s.id === sortBy)?.label || "Recommended";
  ...
  return (
    <div className="flex-1 overflow-x-hidden bg-[#f4f6f8] dark:bg-[#0a0f16] flex flex-col min-h-screen">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, #0f1923 0%, #0a2e1f 40%, #0f1923 100%)",
        }}
      >
        ...
        <div className="relative z-10 px-6 sm:px-10 py-10 lg:py-14 w-full max-w-7xl mx-auto">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-emerald-400/80 text-sm mt-1">
                  Discover amazing places across the Land of Smiles
                </p>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 mt-6"
          >
            {[
              {
                label: activeTab === "place" ? "Places Found" : "Events Found",
                value: meta.total,
                icon:
                  activeTab === "place" ? (
              ...
              <motion.div
                ...
                className="flex items-center gap-3 bg-white/5 backdrop-blur-md border border-white/10 rounded-xl px-3 sm:px-4 py-2.5 overflow-hidden"
              >
                <div className="text-emerald-400 p-1.5 rounded-lg bg-emerald-500/10 shrink-0">
                  {stat.icon}
                </div>
      ...
      <div className="flex flex-1 w-full max-w-7xl mx-auto items-stretch">
        ...
        <motion.aside
          ...
          className="w-[280px] shrink-0 bg-white/60 dark:bg-[#0a0f16]/80 backdrop-blur-xl border-r border-gray-100 dark:border-slate-800 p-6 overflow-y-auto hidden xl:block"
        ...
        <div className="flex-1 p-4 sm:p-6 min-w-0 flex flex-col">
          ...
          <motion.div
            ...
            className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 mb-6 relative z-30"
          >
            {/* Left: Tabs + Search */}
            <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3 flex-1 min-w-0 w-full">
                  ...
                  >
                    {activeTab === tab && (
                      <motion.div
                        ...
                        className="absolute inset-0 bg-gray-900 dark:bg-emerald-600 rounded-lg"
                        transition={{
                          type: "spring",
                          stiffness: 400,
                          damping: 30,
              ...
              <div className="flex-1 w-full min-w-0 relative group">
                ...
                <div className="flex items-center gap-2 bg-white dark:bg-slate-900 rounded-xl px-3.5 py-2.5 border border-gray-200/80 dark:border-slate-800 shadow-sm relative z-10">
                  ...
                  <input
                    type="text"
                    placeholder={`Search ${activeTab}s, provinces...`}
                    value={searchQuery}
                    onChange={(e) => {
                  ...
                  {searchQuery && (
                    <button
                      ...
                      className="p-1 rounded bg-gray-100 hover:bg-gray-200 dark:bg-slate-800"
                    >
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center justify-between gap-3 bg-white dark:bg-slate-900 rounded-xl px-4 py-2.5 border border-gray-200/80 dark:border-slate-800 shadow-sm text-sm text-gray-700 dark:text-slate-200 hover:border-gray-300 transition-colors min-w-[10rem] overflow-hidden"
                  style={{ fontWeight: 600 }}
                >
              ...
              {/* Calendar view toggle - only for events */}
              {activeTab === "event" && (
                <button
                  ...
                  className={`relative p-2.5 rounded-xl transition-all ${
                    viewMode === "calendar"
                      ...
                      : "bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 shadow-sm text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
                  }`}
                  title="Calendar View"
                >
                  <Calendar className="w-4 h-4" />
              ...
              >
                ...
                {activeFilterCount > 0 && (
                  <motion.span
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-emerald-500 text-white text-[10px] rounded-full flex items-center justify-center shadow-sm"
          ...
          <AnimatePresence mode="wait">
            {isLoading ? (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6 flex-1"
              >
                {[...Array(6)].map((_, i) => (
                  <div
                    key={i}
                    className="h-[340px] rounded-3xl bg-gray-200/50 dark:bg-slate-800/50 animate-pulse border border-gray-100 dark:border-slate-800"
                  />
                ))}
              </motion.div>
            ) : isError ? (
              <motion.div
                ...
                className="flex-1 flex flex-col items-center justify-center text-center py-20 bg-red-50 dark:bg-red-900/10 rounded-3xl border border-red-100 dark:border-red-900/30"
              >
              ...
              >
                {items.length > 0 ? (
                  ...
                  <motion.div
                    initial={{ opacity: 0, scale: 0.95 }}
                    animate={{ opacity: 1, scale: 1 }}
                    className="flex-1 flex flex-col items-center justify-center text-center py-20 bg-white/50 dark:bg-slate-900/50 rounded-3xl border border-dashed border-gray-200 dark:border-slate-800"
                  >
                    ...
                    <p
                      className="text-gray-900 dark:text-slate-100 mb-1"
                      style={{ fontWeight: 800, fontSize: "1.25rem" }}
                    >
                      No matches found
                    </p>
                    <p className="text-sm text-gray-500 dark:text-slate-400 mb-6">
                      We couldn't find any {activeTab}s matching your filters
                      and search.
                    </p>
                    <button
                      onClick={() => {
            ...
            >
              ...
              <div className="sticky top-0 bg-white/95 dark:bg-[#0a0f16]/95 backdrop-blur-xl border-b border-gray-100 dark:border-slate-800 p-4 flex items-center justify-between z-10 shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  {activeFilterCount > 0 && (
                    <span
                      className="w-5 h-5 bg-emerald-500 text-white text-[10px] rounded-full flex items-center justify-center"
                      style={{ fontWeight: 800 }}
                    >
                      {activeFilterCount}
                    </span>
                ...
                <button
                  onClick={() => setMobileSidebarOpen(false)}
                  className="w-8 h-8 rounded-xl bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 flex items-center justify-center transition-colors"
                >
              ...
              <div className="sticky bottom-0 bg-white/95 dark:bg-[#0a0f16]/95 backdrop-blur-xl border-t border-gray-100 dark:border-slate-800 p-4 shrink-0">
                ...
                >
                  <Eye className="w-4 h-4" /> Apply Filters ({meta.total}{" "}
                  results)
                </button>
              </div>
            </motion.div>
