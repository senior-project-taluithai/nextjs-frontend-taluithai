...
const CATEGORY_EMOJI: Record<string, string> = {
    Festival: "🎆", Concert: "🎸", Exhibition: "🖼️", Culture: "🛖",
    Food: "🍜", Market: "🛍️", Sport: "🏅", Party: "🎈",
    Music: "🎵", Religion: "🙏"
};
...
export default function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (isLoadingEvent) return <div className="min-h-screen flex flex-col items-center justify-center bg-[#f4f6f8] text-pink-600"><Loader2 className="w-8 h-8 animate-spin mb-4" /><p>Loading event details...</p></div>;
    ...
    const formattedDateRange = startDate.toLocaleDateString('en-US', { day: 'numeric', month: 'short' }) +
        (startDate.getTime() !== endDate.getTime() ? ` - ${endDate.toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' })}` : `, ${startDate.getFullYear()}`);
    ...
    return (
        <div className="flex-1 min-h-screen pb-20 bg-[#f4f6f8]" style={{ scrollBehavior: "smooth" }}>
            ...
            <div className="relative h-[480px] overflow-hidden">
                ...
                >
                    <Image src={coverImage} alt={event.name_en} fill className="object-cover" priority />
                </motion.div>
                ...
                <div className="absolute bottom-0 left-0 right-0 h-1.5"
                    style={{ background: "repeating-linear-gradient(90deg, #ec4899 0px, #ec4899 8px, #db2777 8px, #db2777 16px, #be185d 16px, #be185d 24px)" }}
                />
                ...
                <div className="absolute bottom-0 left-0 right-0 px-8 pb-10 z-10 container mx-auto">
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2.5 mb-4">
                            ...
                            {event.is_highlight && (
                                <motion.span
                                    animate={{ boxShadow: ["0 0 12px rgba(236,72,153,0.3)", "0 0 24px rgba(236,72,153,0.5)", "0 0 12px rgba(236,72,153,0.3)"] }}
                ...
                {scrollY > 350 && (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-sm text-gray-900" style={{ fontWeight: 700 }}>{event.name_en}</p>
                                <div className="flex items-center gap-1.5">
                                ...
                                {[
                                    ...
                                    <Tilt3DCard key={i} className="cursor-default" intensity={4}>
                                        <div className="rounded-xl p-3 hover:bg-gray-50/80 transition-colors text-center group h-full flex flex-col items-center justify-center">
                                            ...
                                            <p className="text-gray-900 group-hover:text-pink-600 transition-colors" style={{ fontSize: "1.1rem", fontWeight: 700 }}>{stat.value}</p>
                                            <p className="text-xs text-gray-400 hidden sm:block whitespace-nowrap overflow-hidden text-ellipsis w-full">{stat.sub}</p>
                                        </div>
                                    </Tilt3DCard>
                                ))}
                            </div>
                            ...
                            <div className="flex gap-1 bg-white rounded-2xl p-1.5 mb-6 shadow-sm border border-gray-100 overflow-x-auto scrollbar-hide">
                                {(["overview", "reviews", "photos"] as const).map((tab) => (
                                    <button
                                        ...
                                        style={activeTab === tab ? { fontWeight: 600, background: "linear-gradient(135deg, #ec4899 0%, #be185d 100%)" } : { fontWeight: 500 }}
                                    >
                                        {tab === "overview" ? "Overview" : tab === "reviews" ? `Reviews` : "Photos"}
                                        {tab === "reviews" && (
                            ...
                            {activeTab === "overview" && (
                                <motion.div key="overview" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}>
                                    ...
                                    <div className="grid grid-cols-2 lg:grid-cols-2 gap-3 mb-5">
                                        {[
                                            { icon: <Clock className="w-4.5 h-4.5 text-blue-500" />, label: "Timing", value: "All Day", bg: "bg-blue-50", border: "border-blue-100" },
                                            { icon: <Calendar className="w-4.5 h-4.5 text-purple-500" />, label: "Date Range", value: formattedDateRange, bg: "bg-purple-50", border: "border-purple-100" },
                                        ].map((info, i) => (
                            ...
                            <Tilt3DCard intensity={2}>
                                <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg transition-shadow">
                                    <div className="relative h-[250px] overflow-hidden z-0">
                                        <Map pos={[event.latitude, event.longitude]} className="h-full w-full" popupContent={event.name_en} />
                                    ...
                                    <div className="p-4">
                                        <div className="grid grid-cols-2 gap-3">
                                            <div className="bg-gray-50 rounded-xl px-3 py-2.5">
                                                ...
                                                <p className="text-sm text-gray-700" style={{ fontWeight: 600 }}>{event.latitude.toFixed(4)}</p>
                                            </div>
                                            <div className="bg-gray-50 rounded-xl px-3 py-2.5">
                                                <p className="text-xs text-gray-400 mb-0.5">Longitude</p>
                                                <p className="text-sm text-gray-700" style={{ fontWeight: 600 }}>{event.longitude.toFixed(4)}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
